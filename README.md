...
| 料金 | 金額そのもの（例 `1,500円`） | 基本コース 3行、集中 2行、美容痩身 2行、小中学生 2行 | 基本コースの金額は FAQ「保険は使えますか」にも書かれている。`4,000円` は集中と美容痩身の両方でヒットする（4行） |
| コースの数 | `4つの施術コース` | 5行 | コース追加時は「5つの施術コース」に一括置換。`4つの` だけで検索すると「4つの理由」も混ざるので注意 |
| コースを追加する | 下の「コース追加チェックリスト」 | — | |
| コースの色 | `style.css`「Courses」の `.course-card--*` 内の4変数 | — | 色そのものは `:root` の `--color-basic*` `--color-beauty*` `--color-kids*` |
...
4. `.course-grid` の `repeat(2, 1fr)` を列数に合わせる（現在は PC 2×2。スマホは自動で1列）
...
- [ ] 祝日の営業有無
- [ ] 料金・コース内容が実際の運用と一致しているか（特に保険適用の範囲、小中学生コースの 2,500円 の扱い、美容痩身コースの所要時間・通院ペース）
- [ ] 広告表現（体験談・効果表現）の法令確認
